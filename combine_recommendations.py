...
def extract_doc_title(filename):
    ...
    Example: 20251013_211400_advanced_ai_possible_futures_arms_race_recommendations.json
    Returns: advanced_ai_possible_futures_arms_race
    """
    # Remove timestamp prefix (YYYYMMDD_HHMMSS_)
    name = filename.split('_', 2)[-1]
    # Remove _recommendations.json suffix
    name = name.replace('_recommendations.json', '')
    return name
...
def combine_recommendations():
    ...
    # Get all JSON files
    json_files = sorted(recommendations_dir.glob('*.json'))

    print(f"Found {len(json_files)} recommendation files")
    ...
    for json_file in json_files:
        ...
        combined_recommendations.append(combined_entry)

    # Write combined file
    output_file = Path('outputs/all_recommendations_combined.json')
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(combined_recommendations, f, indent=2, ensure_ascii=False)

    print(f"\nCombined {len(combined_recommendations)} documents into {output_file}")
