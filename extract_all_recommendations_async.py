"""
Extract recommendations from all AI trajectory documents using concurrent OpenRouter API calls.

This script processes all markdown documents in the docs/ folder (excluding summaries),
extracts structured recommendations using Claude Sonnet 4.5 via OpenRouter API, and saves
the results as timestamped JSON files in outputs/recommendations/.

Usage:
    python extract_all_recommendations_async.py              # Process all documents
    python extract_all_recommendations_async.py --test       # Test mode (first 3 documents)
    python extract_all_recommendations_async.py --skip       # Skip already processed documents
"""
...
PROMPT_TEMPLATE_PATH = "claude/recommendations_prompt.md"


def load_prompt_template() -> str:
    """Load the recommendations extraction prompt template."""
    try:
...
def generate_output_filename(doc_base_name: str, extension: str = "json") -> str:
    ...
    Returns:
        Filename in format: YYYYMMDD_HHMMSS_docname_recommendations.json or .txt
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{timestamp}_{doc_base_name}_recommendations.{extension}"


def save_recommendations_json(data: Dict, doc_base_name: str, output_dir: str) -> bool:
    """
    Save recommendations JSON to output directory.
...
async def make_openrouter_call(
    ...
    async with semaphore:  # This limits concurrent requests
        ...
        headers = {
            ...
            "X-Title": "AI Trajectory Recommendations Extraction",
                ...
                async with session.post(
                    ...
                    if "choices" in data and len(data["choices"]) > 0:
                        ...
                        try:
                            ...
                            print(f"[{doc_index}/{total_docs}] [OK] Response received - {len(recommendations_data.get('recommendations', []))} recommendations extracted")
        ...
        for coro in asyncio.as_completed(tasks):
            ...
            if result:
                ...
                if isinstance(result, dict) and result.get("__raw_output__"):
                    ...
                    # Save as JSON
                    if save_recommendations_json(result, base_name, OUTPUT_DIR):
                        successful += 1
                        print(f"[PROGRESS] {completed_count}/{len(tasks)} completed ({successful} successful, {failed} failed)")
                    else:
                        failed += 1
...
def main():
    ...
    print("\n" + "="*80)
    print("AI Trajectory Recommendations Extraction")
    print("="*80 + "\n")
    ...
    print("[OK] All done! Check the outputs/recommendations/ folder for results.\n")
